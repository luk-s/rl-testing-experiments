...
def is_middle_game_position(board: chess.Board, middle_game_start: int = 15) -> bool:
...
def is_endgame_position(position: Union[str, chess.Board]) -> bool:
...
def get_fens_from_pgn(
    pgn_path: str, mode: str = "all", max_num: Optional[int] = None, use_dict: bool = True
    ...
    with open(pgn_path) as pgn:
        while max_num is None or len(fen_cache) < max_num:
            ...
            for move in game.mainline_moves():
                ...
                if mode == "middlegame" and not is_middle_game_position(board):
                    if is_endgame_position(board):
                        break
                    continue

                elif mode == "endgame" and not is_endgame_position(board):
                    continue
...
def convert_pgn_to_fens(
    pgn_path: str, output_path: str, mode: str = "all", max_num: Optional[int] = None
):
    ...
    with open(output_path, "w") as f:
        for index, fen in enumerate(get_fens_from_pgn(pgn_path, mode=mode, max_num=max_num)):
            if index % 1000 == 0:
...
if __name__ == "__main__":
    ...
    convert_pgn_to_fens(args.pgn_path, args.output_path, mode=args.mode, max_num=args.max_num)
